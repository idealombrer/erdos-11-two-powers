...
python3 typeB_supnorm_check.py    # Bourgain-Chang sup-norm max|S|/(L+1) = 0.37, 0.31, 0.24
python3 typeB_k2_inventory.py         # k=2 inventory: every p^2|N double is a Mersenne square
```

Some scripts sweep a range of $L$ and take up to a couple of minutes (e.g.
`typeB_moments_conditional_check.py`, `typeB_k2_inventory.py`, `typeB_supnorm_check.py`); the rest run in seconds.
